...
function App() {
  ...
  const fetchData = () => {
    fetch('http://localhost:8000/myapp/items/')
  ...
  const handleAddItem = () => {
    const url = selectedItemId
      ? `http://localhost:8000/myapp/items/${selectedItemId}/`
      : 'http://localhost:8000/myapp/items/';
  ...
  const handleDeleteItem = itemId => {
    fetch(`http://localhost:8000/myapp/items/${itemId}/`, {
      method: 'DELETE',
    })
      .then(response => {
